...
diag [] = []
diag grid = (head $ head grid) : (diag $ removeCol $ removeRow grid)


removeRow grid = tail grid

removeCol grid = map tail grid


grid rows = map (\row -> map (\n -> read n::Int) $ words row) rows
